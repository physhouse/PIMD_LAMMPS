...
void Respa::recurse(int ilevel)
{
  ...
  int ifix = modify->find_fix("feynman");
  if (ifix >= 0) modify->fix[ifix]->copy_flevel_f(ilevel);
  ...
  for (int iloop = 0; iloop < loop[ilevel]; iloop++) {
    ...
    if (ilevel == nlevels-1) {
      ...
      if (nflag) {
        ...
        if (triclinic) domain->x2lamda(atom->nlocal);
        domain->pbc();
        //printf("LAMMPS|PBC Called by respa!\n");
  	...
  	if (ifix >= 0) modify->fix[ifix]->post_neighbor(); // added by me Yining
      } else if (ilevel == 0) {
        timer->stamp();
        comm->forward_comm();
        timer->stamp(TIME_COMM);
  ...
  copy_f_flevel(ilevel);
  if (ifix >= 0) modify->fix[ifix]->copy_f_flevel(ilevel);
}
