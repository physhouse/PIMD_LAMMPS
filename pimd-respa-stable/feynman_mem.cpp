...
void FixFeynman::grow_arrays(int nmax)
{
  ...
  {
    memory->grow(f_rp_level[i], input.nbeads, nmax, 3, "fix_feynman:f_rp_level");
  }
}
...
void FixFeynman::copy_arrays(int i, int j,int deflag)
{
  ...
  for(int ii=0; ii<input.nbeads; ii++)
  {
    ...
    for (int level=0; level<nlevels_respa; level++)
    {
	f_rp_level[level][ii][j][0] = f_rp_level[level][ii][i][0];
	f_rp_level[level][ii][j][1] = f_rp_level[level][ii][i][1];
	f_rp_level[level][ii][j][2] = f_rp_level[level][ii][i][2];
...
int FixFeynman::pack_exchange(int i, double *buf)
{
  ...
  for(int ii=0; ii<input.nbeads; ii++)
  {
    ...
    for (int level=0; level<nlevels_respa; level++)
    {
	buf[offset++] = f_rp_level[level][ii][i][0];
	buf[offset++] = f_rp_level[level][ii][i][1];
	buf[offset++] = f_rp_level[level][ii][i][2];

    }
  }
  //printf("Pack sucessful! offset = %d atom %d \n", offset, i);
...
int FixFeynman::unpack_exchange(int nlocal, double *buf)
{
  ...
  for(int ii=0; ii<input.nbeads; ii++)
  {
    ...
    for (int level=0; level<nlevels_respa; level++)
    {
	f_rp_level[level][ii][nlocal][0] = buf[offset++];
	f_rp_level[level][ii][nlocal][1] = buf[offset++];
	f_rp_level[level][ii][nlocal][2] = buf[offset++];

    }
  }

  //printf("Unpack sucessful! offset = %d atom %d \n", offset, nlocal);
  //return size_peratom_cols;
  return offset;
} 
...
void FixFeynman::unpack_restart(int nlocal, int nth)
{
  ...
  for(int ii=0; ii<input.nbeads; ii++)
  {
    ...
    if(xv_restart)
    {
      ...
      for (int level=0; level<nlevels_respa; level++)
      {
	f_rp_level[level][ii][nlocal][0] = extra[nlocal][m++];
	f_rp_level[level][ii][nlocal][1] = extra[nlocal][m++];
	f_rp_level[level][ii][nlocal][2] = extra[nlocal][m++];
      }
    }
    else m+=9 + nlevels_respa*3;
...
int FixFeynman::pack_reverse_comm(int n, int first, double *buf)
{
  ...
  for(int i=first; i<last; i++) 
  {
    for(int ii=0; ii<input.nbeads; ii++)
    {
      ...
      buf[m++] = f_rp[ii][i][1];
      buf[m++] = f_rp[ii][i][2];
      /*printf("NORESPAchecking %d :: pack_reverse_comm first %d last %d\n", i, first, last-1);
      for (int level=0; level<nlevels_respa; level++)
      {
	buf[m++] = f_rp_level[level][ii][i][0];
	buf[m++] = f_rp_level[level][ii][i][1];
	buf[m++] = f_rp_level[level][ii][i][2];
      }
      printf("AFTERchecking %d :: pack_reverse_comm first %d last %d\n", i, first, last-1);*/
    }
  } 
...
void FixFeynman::unpack_reverse_comm(int n, int *list, double *buf)
{
  ...
  for(int i= 0; i<n; i++) 
  {
    ...
    for(int ii=0; ii<input.nbeads; ii++)
    {
      ...
      /*for (int level=0; level<nlevels_respa; level++)
      {
	f_rp_level[level][ii][j][0] = buf[m++];
	f_rp_level[level][ii][j][1] = buf[m++];
	f_rp_level[level][ii][j][2] = buf[m++];
      }*/
    }
  }
}
